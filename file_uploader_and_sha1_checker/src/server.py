...
class Sha1CheckerServicer(sha1_pb2_grpc.Sha1CheckerServicer):
    ...
    def CheckSha1(self, request, context):
        ...
        response.result, response.correct_sha1, response.calculated_sha1 = sha1_calculator.calculate_and_compare_sha1(
            request.filename, request.checksum)
        return response

    def upload(self, request_iterator, context):
        temp_file = ".txt_files/uploaded_file.txt"
        fileUploader.save_to_file(request_iterator, temp_file)
        size_of_file = os.path.getsize(temp_file)
        return sha1_pb2.Reply(length=os.path.getsize(temp_file))
